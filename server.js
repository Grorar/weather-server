...
/** Railway 등 UTC 서버 환경에서도 한국시간(KST, UTC+9) 기준 Date 객체를 반환 */
function nowKST() {
  const now = new Date();
  // UTC 시각 + 9시간
...
function getBaseDateTime() {
  ...
  const h = now.getUTCHours();   // nowKST는 UTC+9를 더한 값이므로 getUTC*로 읽어야 KST 시각이 됨
  ...
  const mo = String(baseDate.getUTCMonth() + 1).padStart(2, '0');
